...
class BTCTransactionTests: XCTestCase {
    ...
    func testOneInOneOutTestnetTx() {
        ...
        encodedSig = try! BTCCurve.shared.appendDERbytes(encodedDERSig: encodedSig, hashType: 0x01, scriptPubKey: utxo.script!.bytes, pubkey: publicKey.bytes)
    ...
    func testTwoInTwoOutTestnetTx() {
        ...
        let signature1: secp256k1_ecdsa_signature = try! BTCCurve.shared.sign(key: privateKey1, message: doubleSha256)
        let publicKey1 = try! BTCCurve.shared.generatePublicKey(privateKey: privateKey1.data)
        var encodedSig1 = try! BTCCurve.shared.encodeDER(signature: signature1)
        encodedSig1 = try! BTCCurve.shared.appendDERbytes(encodedDERSig: encodedSig1, hashType: 0x01, scriptPubKey: utxo1.script!.bytes, pubkey: publicKey1.bytes)
        ...
        let signature2: secp256k1_ecdsa_signature = try! BTCCurve.shared.sign(key: privateKey2, message: doubleSha256)
        let publicKey2 = try! BTCCurve.shared.generatePublicKey(privateKey: privateKey2.data)
        var encodedSig2 = try! BTCCurve.shared.encodeDER(signature: signature2)
        encodedSig2 = try! BTCCurve.shared.appendDERbytes(encodedDERSig: encodedSig2, hashType: 0x01, scriptPubKey: utxo2.script!.bytes, pubkey: publicKey2.bytes)
